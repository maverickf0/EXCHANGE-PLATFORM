...
export class SubscriptionManager{
    ...
    private redisCallbackHandler = (message: string, channel: string)=>{
        ...
        this.reverseSubscriptions.get(channel)?.forEach(s => UserMananger.getInstance().getUser(s)?.emit(parsedMessage));
    }
    ...
    public unsubscribe(userId:string, subscription: string){
        ...
        if(reverseSubscriptions){
            this.reverseSubscriptions.set(subscription, reverseSubscriptions.filter((user)=>user!==userId))
